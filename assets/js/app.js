...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Scrollytelling Guide Logic
  const scenes = {
    sceneReader: document.getElementById("sceneReader"),
    scenePause: document.getElementById("scenePause"),
    sceneThemes: document.getElementById("sceneThemes"),
    sceneLibrary: document.getElementById("sceneLibrary")
  };
  const scrollSteps = document.querySelectorAll('.scroll-step');
  
  if (scrollSteps.length > 0) {
    let rsvpInterval;
    const rsvpWords = ["Read", "faster", "than", "thought.", "Information", "processing", "is", "no", "longer", "limited."];
    let rsvpIdx = 0;
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          if (sceneId === 'sceneReader') {
            rsvpIdx = 0;
            rsvpInterval = setInterval(() => {
              const f = document.getElementById("dFocalPoint");
              if (f) f.textContent = rsvpWords[rsvpIdx];
              rsvpIdx = (rsvpIdx + 1) % rsvpWords.length;
            }, 150);
          } else if (sceneId === 'sceneThemes') {
            const container = document.getElementById("demoScenesContainer");
            ...
            rsvpInterval = setInterval(() => {
              ...
              opts.forEach(o => o.classList.remove('active'));
              opts[themeIdx].classList.add('active');
              
              if (themeIdx === 0) {
                container.style.background = "#000";
                if(text) text.style.color = "#fff";
              } else if (themeIdx === 1) {
                container.style.background = "#F4EFE6";
                if(text) text.style.color = "#4A3F35";
              } else {
                container.style.background = "#1A1A1A";
                if(text) text.style.color = "#E0E0E0";
              }
            }, 1500);
          } else if (sceneId === 'sceneLibrary') {
             // Let it just show the static mocked state
          }
        } else {
          const stepCard = entry.target.querySelector('.step-card');
